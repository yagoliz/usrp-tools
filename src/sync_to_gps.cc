...
void print_notes(void) {
    ...
    std::cout << boost::format("If you would like to use the internal "
                               "reference(TCXO) in other applications, "
                               "you must configure that explicitly.\n");
    std::cout << boost::format("You can no longer select the external SMAs for "
                               "10 MHz or 1 PPS signaling.\n");
...
int UHD_SAFE_MAIN(int argc, char *argv[]) {
    ...
    try {
        ...
        for (size_t mboard = 0; mboard < num_mboards; mboard++) {
            std::cout << "Synchronizing mboard " << mboard << ": " << usrp->get_mboard_name(mboard)
                      << std::endl;
            ...
            if (gps_locked) {
                ...
                std::cerr << "WARNING:  GPS not locked - time will not be accurate "
                             "until locked"
                          << std::endl;
            }
            ...
            uhd::time_spec_t gps_time =
                uhd::time_spec_t(int64_t(usrp->get_mboard_sensor("gps_time", mboard).to_int()));
